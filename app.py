...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    edges = [
        ("Java Core", "Spring Boot"), ("Java Core", "OOP"),
        ("OOP", "Spring Boot"), ("Spring Boot", "REST API"),
        ("Spring Boot", "Spring Security"), ("Spring Boot", "Hibernate/JPA"),
        ("Hibernate/JPA", "PostgreSQL"), ("REST API", "Advanced Java"),
        ("Spring Security", "Advanced Java"), ("Git", "Vue.js"),
        ("Git", "Docker"), ("Docker", "AWS"), ("Docker", "Kubernetes"),
    ...
    fig_tree.add_trace(go.Scatter(
        ...
        line=dict(color="#475569", width=1.5), hoverinfo="none"
    ))
    for n in nodes:
        fig_tree.add_trace(go.Scatter(
            x=[n["x"]], y=[n["y"]],
            mode="markers+text",
            marker=dict(size=38, color=n["color"], line=dict(color="white", width=2)),
            text=[n["id"]], textposition="bottom center",
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 3 — LIVE COACH CHATBOT (Gemini)
# ══════════════════════════════════════════════════════════════════════════════
elif page == "💬 Live Coach":
    st.title("💬 Live Career Coach")
    st.markdown("Chat with your **Digital Twin** — powered by Gemini AI (free)")
    st.divider()
    ...
    if roast_mode:
        ...
        with 15 years of Java experience reviewing Nursultan Akbekov's tech stack.
        Rules:
        - Roast his stack humorously but constructively
        - Mock that he only has Vue.js ("why not React, are you allergic to popularity?")
        - Point out missing Docker/Kubernetes in 2025 is career suicide
        - Be harsh about missing AWS
        - Joke about GPA 3.59 ("almost a 4.0, what happened in that last 0.41?")
        - End EVERY response with one genuine actionable tip
        - Keep it under 150 words, funny and sharp"""
    else:
        system_prompt = """You are a warm, encouraging career coach and Digital Twin 
        for Nursultan Akbekov, a Java Backend Developer from Kyrgyzstan.
        University: Ala-Too International University, GPA 3.59, Software Engineering.
        Current skills: Java Core, Spring Boot, Spring Security, Hibernate/JPA, 
        REST API, PostgreSQL, MySQL, JUnit, Mockito, Git, Vue.js, C#, Generative AI.
        Predicted career: Java Developer (Experienced).
        Missing skills: Advanced Java, Docker, AWS, Kubernetes, Agile.
        Give specific, actionable advice under 150 words. 
        Always reference his actual skills and real goals."""

    if "messages" not in st.session_state:
        st.session_state.messages = []
    if "roast_prev" not in st.session_state:
        st.session_state.roast_prev = roast_mode

    # Clear history when mode switches
    if st.session_state.roast_prev != roast_mode:
        st.session_state.messages = []
        st.session_state.roast_prev = roast_mode
    ...
    if prompt := st.chat_input("Ask your Digital Twin anything..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..." if not roast_mode else "Sharpening insults..."):
                try:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    .wrapped-title { font-size: 2.2em; font-weight: 900; color: #60a5fa; margin-bottom: 5px; }
    .wrapped-subtitle { font-size: 1em; color: #94a3b8; margin-bottom: 30px; }
    .stat-box { background: rgba(255,255,255,0.07); border-radius: 12px; padding: 16px; margin: 8px 0; }
    .stat-label { font-size: 0.75em; color: #64748b; text-transform: uppercase; letter-spacing: 1px; }
    .stat-value { font-size: 1.3em; font-weight: 700; color: #f1f5f9; }
    </style>
    ...
    with col_mid:
        ...
        <div class="wrapped-card">
            <div class="wrapped-title">🎓 Nursultan Akbekov</div>
            <div class="wrapped-subtitle">Software Engineering · Ala-Too International University</div>
            <div class="stat-box">
            ...
            <div class="stat-box">
                ...
                <div class="stat-value">{SEMESTER_DATA['total_credits']} credits · {SEMESTER_DATA['semesters_completed']} semesters</div>
            </div>
            <br>
            <div style="color:#475569; font-size:0.7em;">Generated by Digital Twin Career Engine · 2025</div>
        </div>
        """, unsafe_allow_html=True)

    st.divider()
    st.info("📸 Screenshot the card above to share on LinkedIn!")
